...
#
#             # Check if the row already exists in the database
#             existing_row = session.query(AITestTask).filter_by(
#                 topic=topic,
#                 question=question,
#                 var1=var1,
#                 var2=var2,
...
#
#             # Get the maximum number from the database
#             last_number = session.query(func.max(AITestTask.id)).scalar() or 0
#
#             try:
#                 # Create a new task in the database
#                 new_lesson = AITestTask(
#                     id=last_number + 1,  # Set the new number as the maximum + 1
#                     topic=topic,
#                     question=question,
#                     var1=var1,
...
#                 # Handle unique constraint violation
#                 session.rollback()
#                 existing_lesson = session.query(AITestTask).filter_by(id=number).first()
#                 existing_lesson.topic = topic
#                 existing_lesson.question = question
#                 existing_lesson.var1 = var1
#                 existing_lesson.var2 = var2
